...
>
> **The Problem:** OpenClay running in Docker can't write to `openclaw.json` or volumes due to UID mismatch. The container runs as `node` (UID 1000) but files are owned by `root` (UID 0).
>
...
**Context:** This error occurs when OpenClay running in Docker tries to write configuration files or create directories but lacks permission.
